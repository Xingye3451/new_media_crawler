...
class AbstractCrawler(ABC):
    def __init__(self):
        self.storage_callback: Optional[Callable] = None
    
    def set_storage_callback(self, callback: Callable):
        """设置存储回调函数"""
        self.storage_callback = callback
    
    async def store_to_redis(self, platform: str, data: Dict, data_type: str = "video"):
        """存储数据到Redis"""
        if self.storage_callback:
            await self.storage_callback(platform, data, data_type)
