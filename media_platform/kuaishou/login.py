...
class KuaishouLogin(AbstractLogin):
    ...
    async def check_login_state(self) -> bool:
        """
            检查快手登录状态（严格模式）
            必须有核心认证cookies和会话cookies才认为登录成功
        """
        ...
        for cookie_name in core_cookies:
            ...
            if cookie_value and len(cookie_value) > 10:
                core_found += 1
                utils.logger.debug(f"✓ 快手核心cookie {cookie_name} 存在")
        ...
        for cookie_name in session_cookies:
            ...
            if cookie_value and len(cookie_value) > 20:
                session_found += 1
                utils.logger.debug(f"✓ 快手会话cookie {cookie_name} 存在")
        
        # 严格验证：核心cookies全部存在 + 至少一个会话cookie
        if core_found == len(core_cookies) and session_found >= 1:
            utils.logger.info(f"✅ 快手登录状态检查通过！核心认证({core_found}/{len(core_cookies)}) + 会话({session_found})")
            return True
        else:
            utils.logger.debug(f"快手登录状态检查中... 核心认证({core_found}/{len(core_cookies)}), 会话({session_found})")
            return False
