...
```bash
# 查看支持的平台
python multi_platform_cli.py platforms

# 创建多平台抓取任务
python multi_platform_cli.py create \
    --platforms xhs,dy,ks \
    --keywords "美食探店" \
    --max-count 50 \
    --enable-images \
...
# 查看任务结果
python multi_platform_cli.py results <task_id>
```

### 统一结果格式

每个抓取结果包含：
- **平台信息**：来源平台（小红书、抖音、快手等）
- **内容信息**：标题、作者、发布时间、内容描述
- **互动数据**：点赞数、评论数、分享数、播放量
- **下载链接**：图片、视频等媒体文件链接
- **标签信息**：相关标签和分类

详细使用说明请查看：[多平台抓取使用指南](docs/多平台抓取使用指南.md)
