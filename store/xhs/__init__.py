...
class XhsStoreFactory:
    ...
    def create_store() -> AbstractStore:
        ...
        if not store_class:
            raise ValueError(
                "[XhsStoreFactory.create_store] Invalid save option only supported csv or db or json or redis ..."
            )
        return store_class()
...
def _clear_collected_data():
    ...
    utils.logger.info("[XhsStore] 已清空收集的数据，准备新的爬取任务")
