...
# 添加版本标签
LABEL maintainer="MediaCrawler Team"
LABEL version="${VERSION}"
LABEL description="MediaCrawler API Server with VNC Support"
...
echo "Version: ${APP_VERSION:-$(cat /app/VERSION 2>/dev/null || echo "unknown")}"\n\
echo "Build Date: ${BUILD_DATE:-unknown}"\n\
echo "Git Commit: ${VCS_REF:-unknown}"\n\
' > /usr/local/bin/show-version.sh && \
    chmod +x /usr/local/bin/show-version.sh
...
directory=/app\n\
environment=DISPLAY=":1",PYTHONPATH="/app",APP_VERSION="${APP_VERSION}"\n\
autorestart=true\n\
user=root\n\
